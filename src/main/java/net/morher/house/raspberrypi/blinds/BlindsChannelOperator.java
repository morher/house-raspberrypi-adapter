...
public class BlindsChannelOperator {
  ...
  public BlindsChannelOperator(
      HouseScheduler scheduler,
      BlindsChannel channel,
      CoverEntity entity,
      BlindsRemoteChannelConfig config) {
    ...
    TopicValueHandler<Integer> positionObserver = entity.position().state().observer(0);
  ...
  private void update() {
    ...
    if (isNewPositionOrdered()) {
      Optional<Distance> distance =
          position.calculateDistanceTo(requestedPosition, scheduler.now());
